...
WHEN SHIP:PERIAPSIS = ORBIT:PERIAPSIS THEN {
    LOGTOLOGPATH("DO  MANOEUVER HERE PERIAPSIS "  +"VELOCITY"+ SHIP:VELOCITY ).

}

WHEN SHIP:APOAPSIS = ORBIT:APOAPSIS THEN {
...
UNTIL ORBIT:APOAPSIS >= 150000  {    
        IF ORBIT:APOAPSIS <= 120000 {
            LOCK THROTTLE TO 0.30.
        }
        IF ORBIT:APOAPSIS >= 110000{
...
UNTIL SHIP:ALTITUDE >= ORBIT:APOAPSIS - 30 {
        PRINT_GLOBAL_DATA("SHIP:ALTITUDE >= ORBIT:APOAPSIS              ").
        SET MYSTEER TO SHIP:PROGRADE.
        LOCK THROTTLE TO 0.
}

UNTIL   ORBIT:PERIAPSIS >=155000 {
    PRINT_GLOBAL_DATA("SHIP:ALTITUDE <= ABS(ORBIT:PERIAPSIS)             " ).
    SET MYSTEER TO SHIP:PROGRADE.
    lOCK THROTTLE TO 1.
}
